...
if (isset($_POST['selesai'])) {
	...
	if ($updatestatus) {
		...
		while ($update = mysqli_fetch_assoc($brgs)) {
			$idprod =  $update['idproduk'];
			$updatestatusproduk = mysqli_query($conn, "UPDATE produk SET status='tersedia' WHERE idproduk=$idprod");
		}
...
<head>
	...
	<title>Prima Flora - Pesanan <?php echo $checkdb['namalengkap']; ?></title>
									...
									<table id="dataTable3" class="display" style="width:100%">
										...
										<tbody>
											...
											while ($p = mysqli_fetch_array($brgs)) {
												...
												$result = mysqli_query($conn, "SELECT SUM(p.hargaafter) AS count FROM detailorder d, produk p where orderid='$orderids' and d.idproduk=p.idproduk order by d.idproduk ASC");
										...
										<tfoot>
											<tr>
												...
												<th>Rp<?php
														...
														$result1 = mysqli_query($conn, "SELECT SUM(p.hargaafter) AS count FROM detailorder d, produk p where orderid='$orderids' and d.idproduk=p.idproduk order by d.idproduk ASC");
